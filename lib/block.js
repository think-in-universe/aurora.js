...
export class BlockProxy {
    ...
    static lookup(provider, id) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield provider.block(parseBlockID(id));
                return Ok(true);
            }
            catch (error) {
    ...
    static fetch(provider, id, options) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const block = (yield provider.block(parseBlockID(id)));
                let chunks = [];
                let transactions = [];
                let outcomes = [];
                if (options) {
                    if (options.chunks || options.transactions) {
                        ...
                        chunks = yield Promise.all(requests);
                    }
                    if (options.transactions === 'id') {
                        const requests = chunks.flatMap((chunk) => {
                            return chunk.transactions.map((txHeader) => __awaiter(this, void 0, void 0, function* () {
                                return TransactionID.fromBase58(txHeader.hash);
                            }));
                        });
                        transactions = yield Promise.all(requests);
                    }
                    else if (options.transactions === 'full') {
                        const requests = chunks.flatMap((chunk) => {
                            return chunk.transactions.map((txHeader) => __awaiter(this, void 0, void 0, function* () {
                                return yield provider.txStatus(base58ToBytes(txHeader.hash), txHeader.signer_id);
                            }));
                        });
                        outcomes = yield Promise.all(requests);
                    }
                }
